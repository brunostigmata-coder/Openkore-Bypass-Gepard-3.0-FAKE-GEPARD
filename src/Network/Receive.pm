...
sub received_characters_unpackString {
	my ($self) = @_;
	my $char_info;
	for ($self ? $self->received_characters_blockSize() : ($masterServer && $masterServer->{charBlockSize})) {
...
sub received_characters {
	...
	for (my $i = 0; $i < length($args->{charInfo}); $i += $blockSize) {
		my $temporary_character;
		@{$temporary_character}{@{$char_info->{keys}}} = unpack($char_info->{types}, substr($args->{charInfo}, $i, $blockSize));
		...
		@{$character}{@{$char_info->{keys}}} = unpack($char_info->{types}, substr($args->{charInfo}, $i, $blockSize));
